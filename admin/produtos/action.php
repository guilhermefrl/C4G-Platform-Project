...
if(isset($_POST['addprodutos'])){
    ...
    $prod_nivel = $_POST['prod_nivel'];
    $prod_web = $_POST['prod_web'];
    ...
    $sql3 = "INSERT INTO produtos (rec_id,prod_nivel,prod_web,prod_tipo) VALUES (?,?,?,?)";
    $result3 = sqlsrv_query($conn,$sql3,array($rec_id,$prod_nivel,$prod_web,$prod_tipo));
    if($result3){
        sqlsrv_commit($conn);
        sqlsrv_close($conn);
...
}else if (isset($_POST['editprodutos'])){
    $rec_designacao_pt = $_POST['rec_designacao_pt'];
    $rec_descignacao_en = $_POST['rec_designacao_en'];
    $rec_obs = $_POST['rec_obs'];
    $rec_custo = $_POST['rec_custo'];
    ...
    $sql2 = "UPDATE produtos SET prod_nivel=?, prod_web=?, prod_tipo=? WHERE rec_id = ?";
    $result2 = sqlsrv_query($conn,$sql2,array($prod_nivel,$prod_web,$prod_tipo,$rec_id));
    ...
    echo '<div class="alert alert-danger fade show position-fixed mw-25" style="left:50%;top:20px;transform:translateX(-50%);z-index:100;" role="alert">Erro ao alterar.</div>';
    exit(0);
}else if (isset($_POST['eraseprodutos'])){
    $rec_id = $_POST['eraseprodutos'];
    ...
    $sql1 = "DELETE FROM produtos WHERE rec_id = ?";
    ...
    $sql = "SELECT * FROM produtos AS d  INNER JOIN recurso AS r ON ( d.rec_id = r.rec_id  ) WHERE r.rec_designacao_pt LIKE '%$text%' OR r.rec_designacao_en LIKE '%$text%' OR d.prod_web LIKE '%$text%' OR d.prod_tipo LIKE '%$text%' ;";
