...
if(isset($_POST['adddados'])){
    ...
    $sql3 = "INSERT INTO dados (rec_id,dados_web) VALUES (?,?)";
    $result3 = sqlsrv_query($conn,$sql3,array($rec_id,$dados_web));
    if($result3){
        sqlsrv_commit($conn);
        sqlsrv_close($conn);
...
}else if (isset($_POST['editdados'])){
    $rec_designacao_pt = $_POST['rec_designacao_pt'];
    $rec_descignacao_en = $_POST['rec_designacao_en'];
    $rec_obs = $_POST['rec_obs'];
    $rec_custo = $_POST['rec_custo'];
    $dados_web = $_POST['dados_web'];
    $rec_id = $_POST['editdados'];
    ...
    $sql2 = "UPDATE dados SET dados_web=? WHERE rec_id = ?";
    $result2 = sqlsrv_query($conn,$sql2,array($dados_web,$rec_id));
    ...
    echo '<div class="alert alert-danger fade show position-fixed mw-25" style="left:50%;top:20px;transform:translateX(-50%);z-index:100;" role="alert">Erro ao alterar.</div>';
    exit(0);
}else if (isset($_POST['erasedados'])){
    $rec_id = $_POST['erasedados'];
    ...
    $sql1 = "DELETE FROM dados WHERE rec_id = ?";
    ...
    $sql = "SELECT * FROM dados AS d  INNER JOIN recurso AS r ON ( d.rec_id = r.rec_id  ) WHERE r.rec_designacao_pt LIKE '%$text%' OR r.rec_designacao_en LIKE '%$text%' OR d.dados_web LIKE '%$text%' ;";
    $result = sqlsrv_query($conn,$sql);
    if($result){
        while($row = sqlsrv_fetch_array($result)){
            echo '<tr id="dados_row_'.$row["rec_id"].'">
                    <td>'.$row['rec_designacao_pt'].'</td>
                    <td style="max-width:200px;overflow:auto;">'.$row["dados_web"].'</td>
                    <td>'.$row['rec_custo'].'</td>
                    <td class="d-none">'.$row['rec_designacao_en'].'</td>
                    <td class="d-none">'.$row['rec_obs'].'</td>';
                    echo '<td>
                        <div class="text-center buttons-min-width" >
                            <button type="button" class="btn btn-info" onclick="editar(this.id)" data-toggle="modal" data-target="#editdados" id="dados_'.$row['rec_id'].'_edit">
                            ...
                            <button type="button" class="btn btn-danger" id="dados_'.$row['rec_id'].'_erase" onclick=\'$("#erasedados .btn-danger").val('.$row['rec_id'].');\' data-toggle="modal" data-target="#erasedados">
                            <div class="show-mobile">
